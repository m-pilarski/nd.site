...
callr::r(\(.call_env){
  ...
  if(.call_env$condaenv_name %in% .conda_list$name){
    ...
    .python_path <- (
      .conda_list$python[.conda_list$name == .call_env$condaenv_name]
    )
    ...
    .python_path <- reticulate::conda_create(
      ...
      python_version="3.10",
      environment=(
        if(fs::file_exists("environment.yml")){"environment.yml"}else{NULL}
      )
    )
  ...
  reticulate::conda_install(
    envname=.call_env$condaenv_name, packages="conda-forge::hugo"
  )

  renv::use_python(.python_path, type="conda", name=.call_env$condaenv_name)

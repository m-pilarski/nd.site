...
custom_stopwords_ext <- tibble(Wort = c(
  custom_stopwords$Wort,
  tolower(readr::read_lines("data/german_stopwords_full.txt")),
  "leider","immer","besser","produkt","amazon","bestellt","kunden",
  "gerät","geräte","artikel","funktioniert","super", "jbl", "sony", "gehen", "kaputt", "rechte", "linke",
  "köpfhörer", "absolut", "gutes", "gut", "schlechtes", "schlecht", "erwarten", "hören", "tag", 
  "zweite", "richtig", "lange", "musik", "hörer", "gekauft", "kaufen",
  "produkt", "benutzung", "sterne", "größe", "jahren", "gegangen", "benutze", "kopfhörern", "stern", "preis", "sound", "klang"
)) |>
  distinct() |>
...
stm_model <- stm(
  ...
  vocab     = out$vocab,
  K         = 5,
  prevalence = ~ Bewertung + Produkt + lubridate::dmy(Datum, locale = "de_DE.UTF-8"),    
  data      = out$meta,
  init.type = "Spectral",
  verbose   = FALSE,
  seed      = 123
...
topic_labels <- tibble(
  ...
  label = c("Langlebigkeit", "Verarbeitung", "Passform", "Sound", "Preis-Leistung")[seq_along(unique(top_terms$topic))],
  col   = c("#cf597e", "#e9e29c", "#009392", "#9ccb86", "#eeb479")[seq_along(unique(top_terms$topic))]
)
...
if (interactive()) {
  12 * 0.25 + calc_panel_margin_y_in(topic_plot)
}
...
if (interactive()) {
  4 * 0.25 + calc_panel_margin_y_in(topic_doc_distr)
}
...
if (interactive()) {
  4 * 0.25 + calc_panel_margin_y_in(topic_summary_plot)
}
...
stacked_topics_plot <-
  ggplot(topic_share_q |> 
    ...
    labs(x = NULL, y = "Anteil der Themen") +

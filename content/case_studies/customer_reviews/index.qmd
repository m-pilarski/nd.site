...
tfidf_plot <- review_tokens_clean |>
  ...
  ungroup() |>
  ggplot(aes(x = reorder_within(Wort, tf_idf, Produkttyp),
             y = tf_idf, fill = Produkttyp)) +  
  geom_col(show.legend = FALSE) +
  facet_wrap(~Produkttyp, scales = "free") +
  scale_x_reordered() +
  rcartocolor::scale_fill_carto_d(type = "diverging", palette = "Temps") +
...
{{{< fa-ul >}}}
  {{{< fa-solid-li icon="lightbulb" >}}} Bei Headphones stehen „musik“, „kabelbruch“ und „enttäuschend“ für typische Nutzung und Schwächen. {{{< /fa-solid-li >}}}

  {{{< fa-solid-li icon="lightbulb" >}}} Phone Accessories sind geprägt von Begriffen wie „fritzbox“, „router“ und „reichweite“. {{{< /fa-solid-li >}}}

  {{{< fa-solid-li icon="lightbulb" >}}} Bei Smartphones dominieren Themen wie „kamera“, „apps“ und „wasserdicht“.{{{< /fa-solid-li >}}}
{{{< /fa-ul >}}}
...
id_map <- tibble(doc_id = seq_len(nrow(sentences)), sent_key = sentences$sent_key)

sent_tbl <- vns::calc_tok_sentidict_tbl(
  sentences$sentence, .sentidict_tbl = vns.data::gerpolclu_tbl
) |>
  group_by(doc_id) |>
...
map_aspect <- function(x) {
  ...
  case_when(
    str_detect(x, regex("preis|teuer|g(ü|u)nstig|billig|preis[- ]?leistung|kosten", TRUE)) ~ "Preis",
    str_detect(x, regex("liefer|versand|kundendienst|service|support|r(ü|u)ckgab|antwort|hilfe", TRUE)) ~ "Service",
    str_detect(x, regex("funktion|funktioniert|akku|laufzeit|kompatib|bedien|einstell|app|software", TRUE)) ~ "Funktionalität",
    str_detect(x, regex("qualit(ä|a)t|verarbeitung|material|haltbar|lebensdauer|defekt|kaputt|bruch|kabel|wackelkontakt", TRUE)) ~ "Qualität",
    TRUE ~ NA_character_
  )
}
...
ggplot(aspect_summary,
       aes(x = Aspect, y = avg_sent, fill = Produkttyp)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.65) +
  geom_hline(yintercept = 0, colour = "grey60") +
  rcartocolor::scale_fill_carto_d(type = "diverging", palette = "Temps") +
  labs(x = NULL, y = "Ø Sentiment", fill = "Produkttyp") +
  theme(legend.position = "bottom")
...
Um die zentralen Kritikpunkte in den Rezensionen zur Produktkategorie *Kopfhörer* präziser herauszuarbeiten, berechnen wir im nächsten Schritt ein **Structural Topic Modelling (STM)**. Diese Methode bietet die Möglichkeit andere Faktoren, die die Themen beeinflussen könnten, zu berücksichtigen, wie die Bewertungen der Kund:innen und das Datum, an dem die Rezension verfasst wurde. Für unser Korpus ergeben sich drei Schwerpunkte: Preis‑Leistung, Langfristige Nutzung und Defekte.

Die folgende Grafik zeigt für jedes Thema die repräsentativsten Begriffe. So wird deutlich, warum wir die Themen so benannt haben (z.B. „kaputt“ oder „schad“ im Defekt‑Thema).
...
# stemming 
custom_stopwords_ext <- tibble(Wort = c(custom_stopwords$Wort, tolower(c(read_lines("data/german_stopwords_full.txt"))),
                                        "leider", "immer", "besser"))
...
topic_labels <- tibble(
  topic = 1:3,
  label = c("Preis-Leistung", "Langfristige Nutzung", "Defekte"),
  col = c("#cf597e", "#e9e29c", "#009392")
)
...
topic_doc_distr <- 
  ...
  labs(x = NULL, y = "Wahrscheinlichkeit") +
  scale_fill_manual(values = setNames(topic_labels$col, topic_labels$topic))
...
Das Thema **Defekte** weist die höchste durchschnittliche Dokument‑Wahrscheinlichkeit auf, gefolgt von **Preis‑Leistung** und **langfristige Nutzung**. Das stützt die Diagnose aus der Sentiment‑ und Bigram‑Analyse: Defekte dominieren die Kopfhörer‑Rezensionen inhaltlich.
...
Die Analyse der Sentimentwerte pro Thema zeigt: Die größte Unzufriedenheit äußert sich bezüglich der vermehrt auftretenden Defekten, die meist kurz nach dem Kauf kaputt gehen. Auch die Themen *langfristige Nutzung* und *Preis-Leistung* werden kritisch bewertet, allerdings nicht in dem Maße wie Defekte, die die Soundqualität beeinträchtigen. Eine gezielte Verbesserung der Produktqualität oder des Rückgabeprozesses könnten dazu beitragen, die Kundenzufriedenheit langfristig zu steigern. 

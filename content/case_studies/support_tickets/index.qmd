...
support <-
  read_rds("data/german_customer_support_tickets.rds") |>
  mutate(
    row_id = row_number() |> 
      as.character(),
    .before = subject
...
Dabei entfernen wir sogenannte Stoppwörter, also sehr häufige Wörter ohne inhaltliche Aussagekraft. Zusätzlich bereinigen wir die Texte um wiederkehrende Begrüßungen wie „Sehr geehrtes Support-Team“ oder "Sehr geehrter Kundenservice", da auch diese keinen Beitrag zur inhaltlichen Analyse leisten.
...
custom_stopwords <- tibble(word = tolower(c(
...
support_tokens_clean <- support |>
    mutate(Nachricht = str_replace_all(Nachricht, "\\\\n", " "),
...
if(interactive()){
  7 * 0.25 + calc_panel_margin_y_in(freq_plot)
}
...
```{r}
#| fig-cap: Word Frequency per Urgency Class
#| fig-height: 2.498452
...
freq_per_class_plot <-
  ...
  slice_max(n, n = 10) |>
  ungroup() |>
  ggplot(aes(x = reorder_within(word, n, Dringlichkeit), 
             y = n, fill = Dringlichkeit)) +
...
if(interactive()){
  7 * 0.25 + calc_panel_margin_y_in(freq_per_class_plot)
}
...
```{r}
#| fig-cap: TF-IDF per Urgency Class
#| fig-height: 2.498452
...
tfidf_plot <-
  ...
  ungroup() |>
  ggplot(aes(x = reorder_within(word, tf_idf, Dringlichkeit), 
                        y = tf_idf, fill = Dringlichkeit)) +
  geom_col(show.legend = FALSE) +
...
if(interactive()){
  7 * 0.25 + calc_panel_margin_y_in(tfidf_plot)
}
...
support_clean <- support |>
  mutate(
    ...
    `Betreff vorhanden` = (!is.na(Betreff) & str_length(str_trim(Betreff)) > 0) |> factor(), 
    Nachricht = str_replace_all(Nachricht, "\\\\n", " "),
    Nachricht = str_to_lower(Nachricht),
    Nachricht = str_remove_all(Nachricht, "\\b\\d+\\b|\\b[sa]\\d+\\b|\\b[0-9]+[a-z]*\\b"),
    Nachricht = str_remove_all(Nachricht, paste0("\\b(", paste(custom_stopwords$word, collapse = "|"), ")\\b")),
...
comp_table <- support_clean |>
  ...
  summarise(
    ...
    `Betreff vorhanden (%)` = mean(as.numeric(`Betreff vorhanden`) == 1, na.rm = TRUE) * 100,
    .groups = "drop"
  )
...
support_summary <- support_clean |>
  left_join(
    sentiment_tokens |>
      ...
      summarise(Sentimentwert = sum(tok_pol_num, na.rm = TRUE), .groups = "drop"),
    by = c("ID" = "doc_id")
  ) |>
  mutate(
...
overall_sentiment_plot <-
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Positiv" = "#39b185", 
                               "Neutral" = "#e9e29c", 
                               "Negativ" = "#cf597e"))
...
```{r}
#| fig-cap: Distribution of Sentiment per Urgency Class 
#| fig-height: 2.424034
...
support_model_data <- support_summary |>
  select(Dringlichkeit, Nachricht, Bereich, Sentimentwert, Wortanzahl, Wortlänge, 
         Frage, `Betreff vorhanden`, `Zeichenanzahl`, `Anzahl Ausrufezeichen`) |>
  drop_na() |>
...
final_rf_fit <- readRDS("data/final_rf_model.rds")
...
final_preds <- read.csv("data/final_predictions.csv") |>
  mutate(urgency = factor(urgency),
